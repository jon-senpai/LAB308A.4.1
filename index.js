...
async function loadBreeds() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch cat breeds (Status: ${response.status})`);
    }

    const breedsData = await response.json();

    breedsData.forEach(catBreed => {
      const breedOption = document.createElement('option');
      breedOption.value = catBreed.id;
      breedOption.textContent = catBreed.name;
    ...
    console.error('Encountered an issue while loading cat breeds:', error.message);
  }
}
...
async function handleBreedSelect() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Error: Unable to fetch breed information (Status: ${response.status})`);
    }
